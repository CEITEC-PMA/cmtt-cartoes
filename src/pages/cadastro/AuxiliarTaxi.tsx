...
import imagem from "../../../public/auxiliartaxi.jpg";
...
const AuxiliarTaxi = () => {
  ...
  const handleSave = (dados: any) => {
    ...
    schema
      ...
      .then((dadosValidados) => {
        ...
        axios
          .post("http://localhost:3000/v1/AuxiliarTaxista", dadosValidados)
          .then((response) => {
        ...
        pdf.text(`${dadosValidados.marca}`, 55, 48);
        pdf.text(`${dadosValidados.modelo}`, 55, 58);
        pdf.text(`${dadosValidados.cpf}`, 55, 68);
        pdf.text(`${dadosValidados.rg}`, 55, 78);
        pdf.text(`${dadosValidados.ponto}`, 100, 49);
        pdf.text(`${dadosValidados.chassi}`, 100, 58);
        pdf.text(`${dadosValidados.ano}`, 100, 68);
        pdf.text(`${dadosValidados.orgao}`, 100, 78);
        pdf.text(`${dadosValidados.matricula}`, 145, 48);
        pdf.text(`${dadosValidados.placa}`, 145, 58);
        pdf.text(`${dadosValidados.validade}`, 145, 68);
        pdf.text(`${dadosValidados.dataexpedicao}`, 135, 78);
        pdf.text(`${dadosValidados.endereco}`, 73, 36);
        pdf.text(`${dadosValidados.name}`, 80, 32);
        pdf.text(`${dadosValidados.emissao}`, 116, 88);
        pdf.text(`${dadosValidados.name}`, 76, 90);

        // PARTE D0 MEIO DOS CARTOES
        //primeiro
        pdf.text(`${dadosValidados.matricula}`, 33, 154);
        pdf.text(`${dadosValidados.isncricaomunicipal}`, 33, 162);
        pdf.text(`${dadosValidados.ponto}`, 33, 176);
        pdf.text(`${dadosValidados.validade}`, 33, 182);
        pdf.text(`${dadosValidados.name}`, 33, 168);
        //segundo
        pdf.text(`${dadosValidados.placa}`, 180, 158);
        pdf.text(`${dadosValidados.cor}`, 179, 166);
        pdf.text(`${dadosValidados.name}`, 140, 148);
        pdf.text(`${dadosValidados.marca}`, 140, 167);
        pdf.text(`${dadosValidados.modelo}`, 140, 158);
        pdf.text(`${dadosValidados.chassi}`, 140, 174);
        pdf.text(`${dadosValidados.ano}`, 142, 179);
        pdf.text(`${dadosValidados.validade}`, 175, 180);
        pdf.text(`${dadosValidados.matricula}`, 180, 175);

        pdf.save("auxiliarTaxista.pdf");
  ...
  const schema = yup.object().shape({
    ...
    isncricaomunicipal: yup.string().required().uppercase(),
  });
    ...
    <>
      <UnForm ref={formRef} onSubmit={handleSave}>
        <Grid container display="flex" justifyContent="center">
          <Grid item my={2}>
            <Typography variant="h6">Cadastro Auxiliar de taxista </Typography>
          </Grid>
        </Grid>
        <Grid container display="flex" justifyContent="center">
          <Grid item mb={2}>
